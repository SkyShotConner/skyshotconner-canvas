...
export default function SpecialLocationPage({params}:{params:Promise<{slug:string}>}){
  ...
  return <div className="ssc-special-page">
    ...
    <main className="ssc-special-shell">
      ...
      {!loading&&location&&<><section className="ssc-location-hero"><div><div className="ssc-special-eyebrow">Special Collection / Location</div><h1>{location.name.toUpperCase()}.</h1></div><div><div className="ssc-location-label">{location.location_label}</div><p className="ssc-location-copy">{location.description}</p><div className="ssc-location-purchase-note"><MapPin size={14}/><span>Artwork in this collection is available to purchase only at {location.name}.</span></div></div></section><section className="ssc-special-products"><div className="ssc-special-products-head"><h2>Available artworks.</h2><span>{products.length} pieces</span></div>{products.length>0?<div className="ssc-special-grid">{products.map(product=><article className={`ssc-special-product ${product.orientation}`} key={product.id}>{imageFor(product)?<img src={imageFor(product)} alt={product.name}/>:<div className="ssc-special-empty">Image unavailable</div>}<div className="ssc-special-product-meta"><div className="ssc-special-product-name">{product.name}</div><div className="ssc-special-product-note">{product.note||product.short_description||'Special collection artwork'}</div><div className="ssc-special-location-only"><MapPin size={12}/><span>Only available at {location.name}</span></div>{product.exclusive&&<div className="ssc-special-badge">Location Exclusive</div>}</div></article>)}</div>:<div className="ssc-special-empty">No artworks have been assigned to this location yet. Once a piece is added to the Special Collection, it will appear here automatically.</div>}</section></>}
    </main>
    <Footer/>
  </div>
}
