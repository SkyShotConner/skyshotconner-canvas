...
if (!s.includes('function gaItem(')) {
  const canvasPriceMatch = s.match(/function canvasPrice\(size:string,[^)]*\)\{return [^}]+\}/)
  if (!canvasPriceMatch) throw new Error('Unable to locate canvas price helper for GA4 ecommerce')
  const anchor = canvasPriceMatch[0]
