...
if(!s.includes('const [cartToast,setCartToast]')){
  const statePattern=/const \[products,setProducts\]=useState<Product\[\]>\([^\n]+\),\[cart,setCart\]=useState<CartItem\[\]>\(\[\]\),/
  if(statePattern.test(s)){
    s=s.replace(statePattern,match=>match+"[cartToast,setCartToast]=useState<{name:string;size:string}|null>(null),")
  }else{
    throw new Error('Unable to locate cart state for cart toast')
  }
}

// Show a confirmation whenever an item is successfully added to the cart.
const addPattern=/const add=\(p:Product,size=selectedSize,frame=selectedFrame\)=>setCart\(c=>\{const price=canvasPrice\(size,frame\),key=p\.id\+size\+frame,hit=c\.find\(x=>x\.product\.id\+x\.size\+x\.frame===key\);return hit\?c\.map\(x=>x===hit\?\{\.\.\.x,quantity:x\.quantity\+1\}:x\):\[\.\.\.c,\{product:p,size,frame,quantity:1,price\}\]\}\)/
const addMatch=s.match(addPattern)
if(addMatch && !addMatch[0].includes('setCartToast')){
  const oldAdd=addMatch[0]
  const newAdd=oldAdd.replace(')=>setCart(',')=>{setCart(')+";setCartToast({name:p.name,size});window.setTimeout(()=>setCartToast(null),3200)}"
